...
def edit_routine():
    ...
    if rec.lower() in number_mapping:
        routine_id = number_mapping[rec.lower()]
        found_routine = next((routine for routine in rutinas if routine['id'] == routine_id), None)
        ...
        if found_routine:
            ...
            talk(f'Lo siento, no encontré una rutina con el ID {routine_id}.')
    else:
        talk('Por favor, proporciona un número válido o su equivalente en palabras.')
...
def delete_routine():
    ...
    if rec.lower() in number_mapping:
        ...
        if found_routine:
            ...
            talk(f'Rutina con ID {routine_id} eliminada.')
            save_routines(rutinas)
        else:
            talk(f'Lo siento, no encontré una rutina con el ID {routine_id}.')
    else:
    ...
    if matching_ids:
        ...
        for id in matching_ids:
            if check_time(registro[id]['last_time']):
                ...
                registro[id]['last_time'] = rounded_time.strftime('%Y-%m-%d %H:%M')
                ...
                if registro[id]['count'] == 3:
                    ...
                    if existing_routine is None:
                        ...
                        rutinas.append({'id': id, 'action': action, 'hour': rounded_time.strftime('%H:%M')})
                updated = True
                break
        if rutinas:
            save_routines(rutinas)
        if not updated:
            ...
            rounded_time = round_to_nearest_5_minutes(datetime.datetime.now())
            registro[new_id] = {'action': action, 'count': 1, 'last_time': rounded_time.strftime('%Y-%m-%d %H:%M')}
    else:
        print('Nueva instrucción detectada (nueva):', action)
        id = get_unique_id()
        rounded_time = round_to_nearest_5_minutes(datetime.datetime.now())
        registro[id] = {'action': action, 'count': 1, 'last_time': rounded_time.strftime('%Y-%m-%d %H:%M')}
...
def run():
    if keyboard.is_pressed('enter'):
        ...
        if 'reproduce' in rec:
            ...
            talk("Son las " + hora)
        elif any(keyword in rec for keyword in ['enciende', 'prende', 'apaga','prepara', 'hazme']):
            talk('Okey')
            print('Listo')
        ...
        elif any(keyword in rec for keyword in ['cambiar rutina', 'modificar rutina', 'actualizar rutina', 'actualiza rutina', 'cambia rutina','modifica rutina']):
            edit_routine()
